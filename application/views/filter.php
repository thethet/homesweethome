...
<div class="container content">
	...
	<form method="get" action="<?php echo base_url();?>index.php/maid/search_result" enctype="multipart/form-data">	
				...
				<?php 
							if(isset($nationality)){
								foreach($nationality as $key=>$value):
									if($nationality[$key]==$country['id']){
										echo 'checked="checked"';
									}
								endforeach;
							}
				?> 
				id="filter_nat" name="nationality[]" value="<?php echo $country['id']; ?>"> 
			...
			<div class="filter filter1">
				...
				<input type="radio" 
					<?php 
						...
						?> 
				name="age[]" id="filter_age" value="<?php echo $age; ?>"> <label><?php echo $age; ?></label>
				</div>
				<?php } ?>
				<!--<div class="btndivider"><button type="submit" class="searchbtn">Filter</button></div>-->
			</div>
		...
		<div class="filter-wrap">
			...
			<div class="filter filter1">
				...
				?>
				<div class="divider">
				<input type="checkbox" name="type[]" id="filter_type_maid" <?php 
							if(isset($checktypes)){
								foreach($checktypes as $ckey=>$cvalue):
									if($checktypes[$ckey]==$key){
										echo 'checked="checked"';
									}
								endforeach;
							}
	...
	<div class="col-xs-12 col-sm-9 col-md-9 right-content">
		...
		<?php if($filter_result) {?>
		<?php foreach($filter_result as $data) { ?>
		...
		<?php }
					...
					<div class="col-md-12">
							<div class="alert alert-success fade in" style="text-align:center;background-color:#ff9f02;margin-top:55px;">
								<a href="#" class="close" data-dismiss="alert">&times;</a>
								<strong style="text-align: center;font-family: 'Opensans';font-weight: bold;color:#fff;">Result not found!</strong>
							</div>
					</div>
		<?php } ?>
